...
# ✅ Allow CORS for specific origin (Frontend URL)
...
@app.route("/postcode-check", methods=["POST", "OPTIONS"])
def postcode_check():
    ...
    try:
        ...
        # ✅ Loop through all courts and check if 'postcode' exists
        for court in courts_db.values():
            if "postcode" not in court:
                print(f"⚠ Warning: Missing postcode for court: {court.get('court name', 'Unknown')}")
                continue  # Skip this court if 'postcode' is missing

            destination = court["postcode"]

            print(f"Checking court: {court['court name']} - {destination}")

            try:
                distance, _ = distance_time_between_postcodes(origin, destination)
            except Exception as e:
                print(f"Error fetching distance for {destination}: {e}")
        ...
        print(f"❌ Error in /postcode-check: {e}")
